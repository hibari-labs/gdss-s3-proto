...
validate0("HEAD", Uri, "HTTP/1." ++ _N) ->
    validate_uri(Uri);
validate0("GET", Uri, []) -> %% Simple HTTP/0.9
    validate_uri(Uri);
validate0("GET", Uri, "HTTP/0.9") ->
    validate_uri(Uri);
validate0("GET", Uri, "HTTP/1." ++ _N) ->
    validate_uri(Uri);
validate0("POST", Uri, "HTTP/1." ++ _N) ->
    validate_uri(Uri);
validate0("PUT", Uri, "HTTP/1." ++ _N) ->
    validate_uri(Uri);
validate0("DELETE", Uri, "HTTP/1." ++ _N) ->
    validate_uri(Uri);
validate0("ADDUSER", Uri, "HTTP/1." ++ _N) ->
    validate_uri(Uri);
validate0("TRACE", Uri, "HTTP/1." ++ N) when hd(N) >= $1 ->
    validate_uri(Uri);
validate0(Method, Uri, Version) ->
    {error, {not_supported, {Method, Uri, Version}}}.
